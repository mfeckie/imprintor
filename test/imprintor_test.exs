defmodule ImprintorTest do
  ...
  test "compile_to_pdf with large file - 500 items with images and QR codes" do
    ...
    *Low Stock Items:* #sys.inputs.elixir_data.low_stock_count
    ...
    IO.inspect("Starting large PDF generation test...")
    IO.inspect("Before memory usage: #{:erlang.memory(:total)} bytes")
    ...
    IO.inspect("After memory usage: #{:erlang.memory(:total)} bytes")
  end
  ...
  describe "Direct to file" do
    ...
    test "compile_to_pdf_file" do
      template = "= Direct File Test\n\nThis document tests direct file output."
      data = %{"name" => "Direct File"}

      config = Imprintor.Config.new(template, data)

      case Imprintor.compile_to_pdf_file(config, "test_output.pdf") do
        {:ok, "test_output.pdf"} ->
          assert File.exists?("test_output.pdf")
          {:ok, pdf_binary} = File.read("test_output.pdf")
          assert is_binary(pdf_binary)
    ...
    test "compile_to_pdf_file with invalid path returns error" do
      template = "= Invalid Path Test\n\nThis document tests invalid file path handling."
      data = %{"name" => "Invalid Path"}

      config = Imprintor.Config.new(template, data)

      invalid_path = "/invalid_path/test_output.pdf"
    ...
    test "compile_to_pdf with large file - 500 items with images and QR codes" do
      ...
      *Low Stock Items:* #sys.inputs.elixir_data.low_stock_count
      ...
      items =
        Enum.map(1..1500, fn i ->
          %{
            ...
            sku: "SKU-#{String.pad_leading(Integer.to_string(i), 6, "0")}",
            category:
              Enum.random(["Electronics", "Clothing", "Home & Garden", "Sports", "Books"]),
            price: :rand.uniform(1000) + 10,
            stock_quantity: :rand.uniform(100),
            status: if(:rand.uniform() > 0.8, do: "Low Stock", else: "In Stock"),
            description: "High-quality product #{i} with excellent features and durability.",
      ...
      config = Imprintor.Config.new(template, data)
      IO.inspect("Starting large file PDF generation test...")
      IO.inspect("Before file memory usage: #{:erlang.memory(:total)} bytes")
      ...
      IO.inspect("After file memory usage: #{:erlang.memory(:total)} bytes")
    end
  end
end
